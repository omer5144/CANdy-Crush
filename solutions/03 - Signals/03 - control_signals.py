...
def control_signals(interface: str) -> None:
    ...
    no_signals = can.Message(
        arbitration_id=SIGNALS_ID, is_extended_id=False, data=NO_SIGNALS_DATA
    )
    left_signal = can.Message(
        arbitration_id=SIGNALS_ID, is_extended_id=False, data=LEFT_SIGNAL_DATA
    )
    right_signal = can.Message(
        arbitration_id=SIGNALS_ID, is_extended_id=False, data=RIGHT_SIGNAL_DATA
    )
    both_signals = can.Message(
        arbitration_id=SIGNALS_ID, is_extended_id=False, data=BOTH_SIGNALS_DATA
    )

    with can.Bus(
        channel=interface,
        bustype="socketcan",
        can_filters=[{"can_id": SIGNALS_ID, "can_mask": 0xFFF, "extended": False}],
    ) as bus:
        while not g_stop:
            bus.recv()
            bus.send(no_signals)
    ...
    try:
        while True:
            try:
                g_signals_state = int(
                    input(
                        "Enter signals state: (0 - none, 1 - left, 2 - right, 3 - both):   "
                    )
                )
            except Exception:
                print("Invalid input")
